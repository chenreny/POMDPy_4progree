...
class BeliefNode(object):
    ...
    * Key method is create_or_get_child()
    ...
    def child(self, action, obs):
        node = self.action_map.get_action_node(action)#get ACTION NODE
        if node is not None:
            child_node = node.get_child(obs)  # BELIEF NODE
            if child_node is None:
                return None
            child_node.data.update(child_node.get_parent_belief())# update all_rock_data use parent's parent
            return child_node
        else:
            return None
    ...
    def create_or_get_child(self, action, obs):
        ...
        :return: belief node
        """
        action_node = self.action_map.get_action_node(action)#get action using action_map
        ...
        child_node, added = action_node.create_or_get_child(obs)   # this child_node is belief node
