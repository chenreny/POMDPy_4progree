...
class Agent:
    ...
    def __init__(self, model, solver):
        ...
        self.model = model #pass the rock model construted before to agent
    ...
    def discounted_return(self): # use_tf,  Set if using TensorFlow
    ...
    def multi_epoch(self):
        ...
        for i in range(self.model.n_epochs): # Num of epochs of the experiment to conduct, default 100
    ...
    def run_pomcp(self, epoch, eps):
        ...
        solver = self.solver_factory(self) # first build a belief tree, belief tree has many state particles
       ...
       #the process of change root of belief tree, can think of it as building history(build tree by choose action)
        for i in range(self.model.max_steps): #max_steps Max num of steps per trial/episode/trajectory/epoch, 200
            ...
            action = solver.select_eps_greedy_action(eps, start_time) #eps is tree depth
         ...
         ########################################################################
            ...
            if not step_result.is_terminal or not is_legal:
                solver.update(step_result) #update belief state  and prune, the belief state tree gets changed
