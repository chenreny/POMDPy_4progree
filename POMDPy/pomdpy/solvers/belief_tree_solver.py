...
class BeliefTreeSolver(Solver):
    ...
    def monte_carlo_approx(self, eps, start_time):
        ...
        :return:
        """
        #for search  ## n_sims,For POMCP, this is the num of MC simulations to do at each belief node, 500
        for i in range(self.model.n_sims):
            # Reset the Simulator
            self.model.reset_for_simulation()
            self.simulate(self.belief_tree_index, eps, start_time)#simulate with a belief node
