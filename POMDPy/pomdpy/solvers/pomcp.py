...
class POMCP(BeliefTreeSolver):
    ...
    def select_eps_greedy_action(self, eps, start_time):
        ...
        if self.disable_tree:
            ...
            #simulation
            self.monte_carlo_approx(eps, start_time) #use UCB action here to calculate q value
        return ucb_action(self, self.belief_tree_index, True) #chose best action after simulation use mean_q_value
    ...
    def traverse(self, belief_node, tree_depth, start_time):
        ...
        state = belief_node.sample_particle() #s~B, belief tree does not change, just change sampled state

        #sample action
        action = ucb_action(self, belief_node, False)#argmax action, inside simuation, cal q-value to expand tree
        ...
        step_result, is_legal = self.model.generate_step(state, action) #black box
        ...
        if not step_result.is_terminal or not is_legal:
            ...
            if child_belief_node is not None:
                ...
                delayed_reward = self.traverse(child_belief_node, tree_depth, start_time)#recursion
            else:
                delayed_reward = self.rollout(belief_node) # if child_belief_node is None
            tree_depth -= 1
        else:
